    ...
    <div className="flex flex-wrap gap-4">
        {
            karlCol.map((column: any) => (
                <ul className="border-2 dark:border-neutral-500 md:w-72 rounded-lg mt-4 p-4" key={column.id}>
                  <h2 className="pb-2">{column.name}</h2>
                  {
                    ...
                    column.tasks.map((task : any) => (
                      <li className="border-2  flex gap-8 border-gray-600 rounded-md bg-neutral-900 p-4 mb-2" key={task.id}>
                        ...
                        <WorkerStatusModal helmetId={task.helmetId} name={task.name} className={`${task.status === "normal"? "bg-green-700 hover:bg-green-500" : "bg-yellow-800 hover:bg-yellow-500"} px-2 dark:text-white dark:hover:text-black py-1 mt-4 rounded-xl`}></WorkerStatusModal>
                        </div>
                  ...
                  <div className="border-2 border-gray-600 rounded-md p-4 ">
                      ...
                      <CreateTasksModal className="p-2 hover:bg-yellow-700 bg-yellow-600" setKalrCol={setKalrCol} setSprint={setSprint} columns={columns} projectId={projectId} columnId={column.id}/>
                  </div>
                </ul>
            ))
        }
